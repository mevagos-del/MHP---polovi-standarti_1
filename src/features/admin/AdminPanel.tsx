...
import { importActualPerformanceFromExcel } from '../actuals/importActualPerformance';
import { importDictionariesFromExcel } from '../dictionaries/importDictionaries';
import { exportChangeLogCsv, exportCurrentPlansCsv, exportPlanFactCsv } from '../../services/export/csvExport';
...
function AdminPanel({ dictionaries, onDataChanged, onDictionariesUpdated, refreshToken }: Props) {
  ...
  const [actualImportMessage, setActualImportMessage] = useState('');
  const [actualImportErrors, setActualImportErrors] = useState<string[]>([]);
  ...
  const handleActualImport = async (file: File | undefined) => {
    setActualImportMessage('');
    setActualImportErrors([]);

    if (!file) {
      return;
    }

    const result = await importActualPerformanceFromExcel(file, dictionaries);

    if (!result.ok) {
      setActualImportErrors(result.errors);
      return;
    }

    try {
      await activeStorageAdapter.importActualPerformance(result.rows);
      setActualImportMessage('Фактичне виконання успішно імпортовано.');
      onDataChanged();
    } catch {
      setActualImportErrors(['Не вдалося імпортувати фактичне виконання у сховище даних.']);
    }
  };
  ...
  const handleExportPlanFact = async () => {
    if (!requireExportMonth()) {
      return;
    }

    const records = await activeStorageAdapter.getPlanFactByMonth(exportMonthCode);
    exportPlanFactCsv(records, dictionaries, exportMonthCode);
  };
        ...
        <div className="admin-grid">
          ...
          <article className="panel admin-card">
            <div className="section-heading">
              <div>
                <p className="section-kicker">Excel</p>
                <h3>Імпорт фактичного виконання</h3>
              </div>
            </div>
            <p className="helper-text">
              Завантажте файл “Field Standards - Actuals.xlsx” з аркушем Actuals.
              Обовʼязкові колонки: period, channelCode, fullName, actualAuditsStoreChecks,
              actualAdministrativeDays, actualNegotiations, comment.
            </p>
            <input
              accept=".xlsx,.xls"
              onChange={(event) => handleActualImport(event.target.files?.[0])}
              type="file"
            />
            {actualImportMessage && <div className="message message--success">{actualImportMessage}</div>}
            {actualImportErrors.length > 0 && (
              <div className="message message--error">
                {actualImportErrors.map((error) => <p key={error}>{error}</p>)}
              </div>
            )}
          </article>
