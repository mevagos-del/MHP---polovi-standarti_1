          ...
          <article className="analytics-card">
            ...
            <div className="metric-list">
              {latestMetrics.map((metric) => (
                <section className={`metric-row ${metric.key === 'audits' ? 'metric-row--primary' : ''}`} key={metric.key}>
                  <div className="metric-row__top">
                    <strong>{metric.label}</strong>
                    <span className="status-badge">{metric.status}</span>
                  </div>
                  <div className="metric-row__values">
                    <span>План: {metric.plan}</span>
                    <span>Факт: {metric.actual ?? 'Факт ще не завантажено'}</span>
                    <span>Виконання: {metric.completion === undefined ? '—' : `${metric.completion}%`}</span>
                  </div>
                  <div className="progress-track" aria-hidden="true">
                    <div className="progress-fill" style={{ width: `${Math.min(metric.completion ?? 0, 120)}%` }} />
                  </div>
                  <p>{metric.hint}</p>
                </section>
              ))}
