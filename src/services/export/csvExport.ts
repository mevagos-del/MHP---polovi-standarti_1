import type { ChangeLogRecord, CurrentPlan, Dictionaries, PlanFactRecord } from '../../types/domain';
...
function completionPercent(planValue?: number, actualValue?: number) {
  if (planValue === undefined || actualValue === undefined || planValue === 0) {
    return '';
  }

  return Math.round((actualValue / planValue) * 100);
}

export function exportPlanFactCsv(records: PlanFactRecord[], dictionaries: Dictionaries, monthCode: string) {
  const lookups = createLookups(dictionaries);
  const monthName = lookups.months.get(monthCode) ?? monthCode;
  const headers = [
    'Канал збуту',
    'ПІБ',
    'Місяць планування',
    'План Аудити / Сторчеки',
    'Факт Аудити / Сторчеки',
    'Виконання Аудити %',
    'План Адміністративні дні',
    'Факт Адміністративні дні',
    'Виконання Адмін дні %',
    'План Перемовини',
    'Факт Перемовини',
    'Виконання Перемовини %',
    'Коментар плану',
    'Коментар факту',
    'Версія плану',
    'Останнє оновлення плану',
    'Останнє оновлення факту',
  ];
  const rows = records.map((record) => ({
    'Канал збуту': lookups.channels.get(record.channelCode) ?? record.channelCode,
    'ПІБ': lookups.employees.get(record.employeeId) ?? record.employeeId,
    'Місяць планування': monthName,
    'План Аудити / Сторчеки': record.plan?.auditsCount ?? 'Немає плану',
    'Факт Аудити / Сторчеки': record.actual?.actualAuditsCount ?? '',
    'Виконання Аудити %': completionPercent(record.plan?.auditsCount, record.actual?.actualAuditsCount),
    'План Адміністративні дні': record.plan?.adminDaysCount ?? 'Немає плану',
    'Факт Адміністративні дні': record.actual?.actualAdminDaysCount ?? '',
    'Виконання Адмін дні %': completionPercent(record.plan?.adminDaysCount, record.actual?.actualAdminDaysCount),
    'План Перемовини': record.plan?.negotiationsCount ?? 'Немає плану',
    'Факт Перемовини': record.actual?.actualNegotiationsCount ?? '',
    'Виконання Перемовини %': completionPercent(record.plan?.negotiationsCount, record.actual?.actualNegotiationsCount),
    'Коментар плану': record.plan?.comment ?? '',
    'Коментар факту': record.actual?.comment ?? '',
    'Версія плану': record.plan?.version ?? '',
    'Останнє оновлення плану': record.plan?.updatedAt ?? '',
    'Останнє оновлення факту': record.actual?.updatedAt ?? '',
  }));

  downloadCsv(`Field Standards - Plan Fact - ${monthName}.csv`, headers, rows);
}
