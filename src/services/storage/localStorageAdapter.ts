...
function getActualPerformance() {
  return readJson<ActualPerformance[]>(storageKeys.actualPerformance, []);
}

function getActualKey(record: Pick<ActualPerformanceInput, 'channelCode' | 'employeeId' | 'monthCode'>) {
  return `${record.channelCode}::${record.employeeId}::${record.monthCode}`;
}

function joinPlanFact(plans: CurrentPlan[], actuals: ActualPerformance[], includeActualsWithoutPlan = false): PlanFactRecord[] {
  const actualByKey = new Map(actuals.map((actual) => [getActualKey(actual), actual]));
  const records: PlanFactRecord[] = plans.map((plan) => ({
    plan,
    actual: actualByKey.get(getActualKey(plan)) ?? null,
  ...
  if (includeActualsWithoutPlan) {
    const planKeys = new Set(plans.map((plan) => getActualKey(plan)));
    actuals.forEach((actual) => {
      if (!planKeys.has(getActualKey(actual))) {
        records.push({
          plan: null,
          actual,
...
export const localStorageAdapter: StorageAdapter = {
  ...
  async getActualPerformanceByEmployee(channelCode, employeeId) {
    return getActualPerformance().filter((record) => record.channelCode === channelCode && record.employeeId === employeeId);
  },
  async getActualPerformanceByMonth(monthCode) {
    return getActualPerformance().filter((record) => record.monthCode === monthCode);
  },
  async importActualPerformance(rows) {
    const existing = getActualPerformance();
    const nextRecords = [...existing];

    rows.forEach((row) => {
      const existingIndex = nextRecords.findIndex((record) => getActualKey(record) === getActualKey(row));
      const now = formatDateTime();
    ...
    writeJson(storageKeys.actualPerformance, nextRecords);
  },
  async getPlanFactByEmployee(channelCode, employeeId) {
    const plans = getCurrentPlans().filter((plan) => plan.channelCode === channelCode && plan.employeeId === employeeId);
    const actuals = getActualPerformance().filter((record) => record.channelCode === channelCode && record.employeeId === employeeId);
    return joinPlanFact(plans, actuals).sort((a, b) => a.monthCode.localeCompare(b.monthCode));
  },
  async getPlanFactByMonth(monthCode) {
    const plans = getCurrentPlans().filter((plan) => plan.monthCode === monthCode);
    const actuals = getActualPerformance().filter((record) => record.monthCode === monthCode);
